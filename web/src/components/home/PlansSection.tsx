...
function PlanCard({ plan, index }: { plan: ClubPlan; index: number }) {
  ...
  return (
    <FadeIn
      ...
      delay={index * 0.06}
      className={cn(
        'relative flex flex-col rounded-[1.75rem] p-7 sm:p-8',
        featured
          ? 'panel-gold order-first shadow-[0_40px_90px_-45px_rgb(212_175_55/0.45)] lg:order-none lg:px-9 lg:py-12'
          : 'panel',
      )}
    >
      {featured && (
        <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-foil px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.14em] text-obsidian">
          Mais escolhido
        </span>
      )}
  ...
  return (
    ...
    >
      ...
      <div className="container-luxe">
        ...
        <ul className="mx-auto mt-16 grid max-w-md gap-8 lg:max-w-none lg:grid-cols-3 lg:items-center lg:gap-5 xl:gap-7">
